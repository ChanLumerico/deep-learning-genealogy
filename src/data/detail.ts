...
/**
 * The curated journeys offered on arrival, from public/data/paths.json.
 * Its own fetch rather than `load`, because that one falls back to `{}` and
 * this file is an array — a failed load must still be iterable.
 */
let pathsOnce: Promise<unknown[]> | null = null
export function loadPaths(): Promise<unknown[]> {
  pathsOnce ??= fetch(url('data/paths.json'))
    .then((r) => (r.ok ? r.json() : []))
...
/**
 * The search index over every essay, built by tools/gen-search-index.mjs.
 * 181 kB gzipped, so it waits until someone actually searches.
 */
let searchOnce: Promise<unknown> | null = null
export function loadSearchIndex(): Promise<unknown> {
  searchOnce ??= fetch(url('data/search-index.json'))
